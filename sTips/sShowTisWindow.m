...
- (void)setType:(DsTipsWindowType)type{
    ...
    switch (type) {
        ...
        {
            self.tipsWindow.frame = CGRectMake(0, 64, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 64);
        }
            break;
        case tipsWindowExpectTabBar:
        {
            self.tipsWindow.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 49);
        }
            break;
        case tipsWindowExpectBar:
        {
            self.tipsWindow.frame = CGRectMake(0, 64, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 64 - 49);
        }
            break;
        default:
            break;
...
- (void)showTip{
    [UIView animateWithDuration:0.25 animations:^{
        ...
        self.tipsV.frame = CGRectMake(0, 0, self.tipsWindow.frame.size.width, 64);
    } completion:^(BOOL finished) {
        [UIView animateWithDuration:0.25 animations:^{
            self.tipsV.frame = CGRectMake(0, -64, self.tipsWindow.frame.size.width, 64);
...
- (UIWindow *)tipsWindow{
    ...
    if (!_tipsWindow) {
        ...
        _tipsWindow.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.6];
        _tipsWindow.alpha = 0;
    }
    return _tipsWindow;
}
